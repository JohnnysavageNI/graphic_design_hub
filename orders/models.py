...
class DesignRequest(models.Model):
    ...
    uploaded_file = models.FileField(upload_to="user_inputs/%Y/%m/%d/", blank=True, null=True)
    STATUS_CHOICES = [("pending", "Pending"), ("in_progress", "In Progress"), ("completed", "Completed")]
    status = models.CharField(max_length=20, choices=STATUS_CHOICES, default="pending")
    def __str__(self): return f"DesignRequest #{self.pk} — {self.service.name}"
    @property
    def uploads_count(self) -> int: return self.uploads.count()
...
class OrderUpload(models.Model):
    ...
    def __str__(self): return f"Upload #{self.pk} for Request #{self.request_id}"
